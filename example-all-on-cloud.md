...
The examples files and steps below assume that you are using the "default" network.  If you are using a network that you have created yourself you may need to edit the Terraform files to use your network instead of default.  Additionally there are a number of firewall rules that need to be opened to ensure that the Compute Instances can talk to each other and the NFS volumes.  They are as follows:
